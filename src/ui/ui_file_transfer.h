...
static void status_update_timer_cb(lv_timer_t *timer) {
  ...
  if (wifi_get_connected_status()) {
    ...
    lv_obj_set_style_text_color(label_status, lv_color_hex(UI_COLOR_SUCCESS), 0);
    ...
    lv_obj_set_style_text_color(label_status, lv_color_hex(UI_COLOR_WARNING), 0);
...
void ui_file_transfer_init(void) {
  ...
  // Separator
  lv_obj_t *sep = ui_create_h_separator(main_left, lv_color_hex(UI_SEPARATOR_COLOR));
  ...
  // Bouton exit
  lv_obj_t *btn_cancel_pilot = ui_create_button(btn_container, txt->exit, LV_SYMBOL_BACKSPACE, lv_color_hex(UI_COLOR_BTN_CANCEL),
                                                UI_BTN_PRESTART_W, UI_BTN_PRESTART_H, UI_FONT_SMALL, UI_FONT_NORMAL, btn_exit_cb,
